...
/**
 * ISAgentPool provides a means of acquiring initialized Agents in a thread safe manner that
 * minimizes the overhead of constructing and initiating Agents on demand. Agents should be returned
 * to with returnAgent() when they are no longer needed for an operation.
 */
public class ISAgentPool {
   ...
   *         {@link com.ionic.sdk.device.profile.persistor.DeviceProfilePersistorBase Persistor} is
   *         unset or does not contain an active profile.
   */
  ...
  /**
   * returnAgent() returns an Agent to the AgentPool It is no longer to safe to reference the agent
   * afterwards
   *
  ...
  /**
   * setPersistor() sets the persistor to use when making new agents and empties the AgentPool
   *
