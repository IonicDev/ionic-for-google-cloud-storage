...
public class GoogleIonicStorage implements Storage {
  ...
  /**
   * GoogleIonicStorage() constructor for GoogleIonicStorage that takes an existing instance of
   * {@link com.google.cloud.storage.Storage} for performing the underlying Storage operations and a
   * persistor that is set on the object's agentPool.
   *
  ...
  public Blob create(BlobInfo blobInfo, byte[] content, CreateKeysRequest.Key key,
    ...
    return googleStorage.create(pair.info, content, targetOptionsWithEncrytion(pair.key, options));
  }
   ...
   * {@link #create(BlobInfo, byte[], CreateKeysRequest.Key, Storage.BlobTargetOption...)} instead.
   *
   * @return a [@code Blob} with complete information
   * @param blobInfo a {@link com.google.cloud.storage.BlobInfo}
  ...
  public Blob create(BlobInfo blobInfo, InputStream content, CreateKeysRequest.Key key,
    ...
    return googleStorage.create(pair.info, content, writeOptionsWithEncrytion(pair.key, options));
  }
  ...
  private GetKeysResponse.Key ionicKeyFromBlob(BlobId blobId) {
    ...
    try {
      ...
      throw new StorageException(AgentErrorModuleConstants.ISAGENT_MISSINGVALUE,
          "Ionic Key not present in blob metadata.");
    }
  }
  ...
  private GetKeysResponse.Key getIonicKey(String keyid) throws IonicException {
    Agent agent = agentPool.getAgent();
    GetKeysResponse.Key response = null;
    try {
      GetKeysResponse keysResponse = agent.getKey(keyid);
      response = keysResponse.getFirstKey();
    } finally {
      agentPool.returnAgent(agent);
    }
    return response;
