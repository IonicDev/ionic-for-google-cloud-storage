...
import static com.ionic.cloudstorage.gcs.GoogleIonicStorage.IONICMETACONSTANT;
...
public class GoogleIonicStorageFunctionalTest {
    ...
    public static void init()
        ...
        ionicStorage = new GoogleIonicStorage(profilePersistor,
                StorageOptions.getDefaultInstance().getService());
    }

    @Test
    public void downloadUnprotectedFile() throws UnsupportedEncodingException {
        // create using GCS API to ensure Ionic applies no protections
        Storage storage = StorageOptions.getDefaultInstance().getService();
        BlobId blobId = BlobId.of(bucketName, "unprotected-data.txt");
        BlobInfo unprotectedBlobInfo = BlobInfo.newBuilder(blobId).setContentType("text/plain").build();
        Blob blob = storage.create(unprotectedBlobInfo, "unprotected data".getBytes("utf-8"));

        byte[] blobBytes = ionicStorage.readAllBytes(bucketName, "unprotected-data.txt");
        assertArrayEquals("unprotected data".getBytes("utf-8"), blobBytes);
    ...
    public void testMetaDataDestruction() throws IOException {
        ...
        assertArrayEquals(message, oriBlobBytes, blobBytes);
        BlobInfo updatedBlobInfo =
                BlobInfo.newBuilder(blobInfo.getBlobId()).setMetadata(null).build();
        ionicStorage.update(updatedBlobInfo);
        blobBytes = ionicStorage.readAllBytes(bucketName, blobName);
    }
